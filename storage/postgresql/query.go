...
func (b PostgresBackend) QueryEventsUnion(filters nostr.Filters) (events []nostr.Event, err error) {
	var params []any
	if len(filters) == 0 {
		err := fmt.Errorf("no filters")
		return nil, err
	}
	if len(filters) > 10 {
		err := fmt.Errorf("maximum 10 filters")
		return nil, err
	}
...
func (b PostgresBackend) QueryEvents(filter *nostr.Filter) (events []nostr.Event, err error) {
	...
	return b.QueryEventsUnion(nostr.Filters{*filter})
}
